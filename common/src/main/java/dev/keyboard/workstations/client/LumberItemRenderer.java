 ...
 */
public class LumberItemRenderer implements BuiltinItemRenderers.DynamicItemRenderer {
	@Override
	public void render(ItemStack stack, ModelTransformationMode mode, MatrixStack matrices,
			VertexConsumerProvider vertexConsumers, int light, int overlay) {
		MinecraftClient client = MinecraftClient.getInstance();
		client.getBlockRenderManager().renderBlockAsEntity(WorkstationsMod.LUMBER_BLOCK.get().getDefaultState(),
				matrices, vertexConsumers, light, overlay);
		AxeDisplay.render(client.world, client.getRenderTickCounter().getTickDelta(false),
				matrices, vertexConsumers, light, overlay);
	}
}
