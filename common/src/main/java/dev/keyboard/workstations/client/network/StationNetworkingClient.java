...
public final class StationNetworkingClient {
	...
	public static void registerClientReceivers() {
		NetworkManager.registerReceiver(NetworkManager.s2c(), OpenScreenPayload.ID, OpenScreenPayload.CODEC,
				(payload, context) -> context.queue(() ->
						openScreen(MinecraftClient.getInstance(), payload.pos(), payload.palette())));
	}
	...
	public static void saveRanchSettings(BlockPos pos, StationSettings settings) {
		...
		NetworkManager.sendToServer(new SaveSettingsPayload(pos, nbt));
	}
	...
	public static void recallWorker(BlockPos pos) {
		NetworkManager.sendToServer(new RecallWorkerPayload(pos));
	}

	public static void rescanPlots(BlockPos pos) {
		NetworkManager.sendToServer(new RescanPlotsPayload(pos));
	}
}
