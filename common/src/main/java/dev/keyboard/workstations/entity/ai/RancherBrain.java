...
public class RancherBrain {
	...
	private static Stock.Held findFeed(List<Container> stores, Animal animal) {
		...
		return liked != null ? liked : Stock.find(stores, stack -> stack.is(WorkstationsMod.UNIVERSAL_FEED.get()));
	}

	/** Whether one of these is a helping this animal will accept. */
	private static boolean feeds(ItemStack stack, Animal animal) {
		return !stack.isEmpty() && (animal.isFood(stack) || stack.is(WorkstationsMod.UNIVERSAL_FEED.get()));
	}
