...
public class RancherBrain {
	...
	private static Stock.Held findFeed(List<Inventory> stores, AnimalEntity animal) {
		...
		return liked != null ? liked : Stock.find(stores, stack -> stack.isOf(WorkstationsMod.UNIVERSAL_FEED.get()));
	}
	...
	private static boolean feeds(ItemStack stack, AnimalEntity animal) {
		return !stack.isEmpty() && (animal.isBreedingItem(stack) || stack.isOf(WorkstationsMod.UNIVERSAL_FEED.get()));
	}
