...
public class LumberBlockEntityRenderer implements BlockEntityRenderer<LumberBlockEntity> {
	...
	private static void tile(VertexConsumer buffer, Matrix4f matrix, BlockPos origin, BlockPos stump) {
		...
		buffer.vertex(matrix, minX, y, minZ).color(STUMP[0], STUMP[1], STUMP[2], ALPHA);
		buffer.vertex(matrix, minX, y, maxZ).color(STUMP[0], STUMP[1], STUMP[2], ALPHA);
		buffer.vertex(matrix, maxX, y, maxZ).color(STUMP[0], STUMP[1], STUMP[2], ALPHA);
		buffer.vertex(matrix, maxX, y, minZ).color(STUMP[0], STUMP[1], STUMP[2], ALPHA);
	}
}
