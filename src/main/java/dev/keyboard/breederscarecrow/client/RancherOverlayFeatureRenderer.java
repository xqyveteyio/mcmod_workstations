...
/**
 * Draws the rancher model a second time with another texture, the way vanilla layers a villager's
 ...
 */
public class RancherOverlayFeatureRenderer
		extends FeatureRenderer<RancherEntity, VillagerResemblingModel<RancherEntity>> {
	private final Identifier texture;

	public RancherOverlayFeatureRenderer(
			FeatureRendererContext<RancherEntity, VillagerResemblingModel<RancherEntity>> context,
			Identifier texture) {
		super(context);
	...
	public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light,
			RancherEntity entity, float limbAngle, float limbDistance, float tickDelta,
		...
		this.getContextModel()
				.render(matrices, consumer, light, OverlayTexture.DEFAULT_UV, 1.0F, 1.0F, 1.0F, 1.0F);
	}
}
